...
Vec3 CalcAngle(Vec3& origin, Vec3& target)
{
    ...
    results.y = RadiansToDegrees(-atan2f(target.x - origin.x, target.y - origin.y)); // yaw calc
    if (results.y <= 90.0f)
    {
        results.y += 360.0f;
    }
    ...
    results.x = -RadiansToDegrees(asinf((target.z - origin.z) / origin.Distance(target))); // pitch calc
    return results;
}
